...
@Slf4j
public class ExportSupportToPdf {

	private List<Support> supportList;

//	private RestTemplate restTemplate;

	public ExportSupportToPdf(List<Support> supportList) {
	...
	private void writeTableHeader(PdfPTable table) {
		...
		cell.setPhrase(new Phrase("TicketType", font));
		table.addCell(cell);

		cell.setPhrase(new Phrase("TicketDescription", font));
		table.addCell(cell);
	...
	private void writeTableData(PdfPTable table) {
		...
		for (Support support : supportList) {
			...
			cell.setPhrase(new Phrase(support.getTicketType(), font));
			table.addCell(cell);

			cell.setPhrase(
					new Phrase(support.getTicketDescription() != null && !support.getTicketDescription().isBlank()
							? support.getTicketDescription()
							: "Nill", font));
			table.addCell(cell);

			cell.setPhrase(new Phrase(support.getUser().getEmail(), font));
			table.addCell(cell);

			cell.setPhrase(new Phrase(DateUtil.convertDateToStringDateTime(support.getCreationDate()), font));
			table.addCell(cell);

			if (support.getUpdationDate() != null) {
				cell.setPhrase(new Phrase(DateUtil.convertDateToStringDateTime(support.getUpdationDate()), font));
				table.addCell(cell);
			} else {
...
//		log.info("<<<<<<<<<< Added >>>> image in document  >>>>>> : " + document);
		...
		Paragraph p = new Paragraph("List of Support Details", font);
